...
- 实时显示麦克风分贝值（映射到 0–150 范围以便在 UI 中直观显示），**默认仅使用系统实时计量（AudioMeterInformation）读取，不打开麦克风录音通道**，不会触发 Windows 的“麦克风正在使用”提示。
- 校准功能：根据目标 dBFS 自动计算并保存放大倍数（组件设置 → `Magnification`）。校准时才会短暂打开录音通道一次。
...
- `EnableFallbackSampling` (bool)：是否允许“回退录音采样”，默认 false。开启后仅在系统实时计量读不到有效信号时周期性短暂录音取峰值，会占用麦克风通道（供不提供实时计量的设备使用）。
...
- 分贝值长期显示 `0.0`：
  - 默认模式只使用系统实时计量；若你的设备不提供实时计量，可到 **设置 → Decibel_Monitor 页** 开启“回退录音采样”（会周期性占用麦克风）。
  - 确认宿主应用有麦克风权限（Windows 隐私设置）。
  - 确认默认捕获设备已启用并未被独占。
  - 在 **设置 → Decibel_Monitor 页** 使用“捕获设备列表”检查系统设备；增加音量或靠近声源再测。
- 出现“麦克风正在使用”提示且图标闪烁：说明开启了“回退录音采样”，或加载了旧版本插件。请关闭该开关并更新插件；实时监测仅使用系统计量，不会占用麦克风。
- 校准结果异常：
