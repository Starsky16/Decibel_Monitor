...
{
    private readonly IAlertDecisionSource[] _sources;
    ...
    public AlertDecisionCoordinator(IEnumerable<IAlertDecisionSource> sources, IReadOnlyList<string>? sourcePriorityOrder = null)
    {
        ...
        if (sourcePriorityOrder is { Count: > 0 })
        {
            var ordered = new List<IAlertDecisionSource>(list.Length);
            foreach (var id in sourcePriorityOrder)
            {
                var matched = list.FirstOrDefault(s => s.Id == id);
            ...
            // 仅追加未被优先级顺序覆盖的判定源（保持稳定顺序）：缺失条目不代表要禁用
